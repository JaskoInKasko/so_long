...
void	ft_get_image(t_game *game, t_image *image)
{
	...
	image->coin = mlx_xpm_file_to_image(game->mlx, "images/Collectible.xpm", &p, &p);
...
void	ft_image_type(t_game *game, t_map_data *map, t_image *image)
{
	...
	if (current == '0')
		mlx_put_image_to_window(game->mlx, game->mlx_win, image->floor, map->y * PXL, map->x * PXL);
	else if (current == '1')
		mlx_put_image_to_window(game->mlx, game->mlx_win, image->wall, map->y * PXL, map->x * PXL);
	else if (current == 'P')
		mlx_put_image_to_window(game->mlx, game->mlx_win, image->player_down, map->y * PXL, map->x * PXL);
	else if (current == 'C')
		mlx_put_image_to_window(game->mlx, game->mlx_win, image->coin, map->y * PXL, map->x * PXL);
	else if (current == 'E')
		mlx_put_image_to_window(game->mlx, game->mlx_win, image->exit_close, map->y * PXL, map->x * PXL);
}
